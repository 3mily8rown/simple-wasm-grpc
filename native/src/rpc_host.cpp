...
int main() {
    ...
    // … read clientBytes / serverBytes …

    wamr::Module clientMod{client_buffer};
    wamr::Module serverMod{server_buffer};

    auto* client_start = wasm_runtime_lookup_function(clientMod.inst(), "_start");
    auto* server_start = wasm_runtime_lookup_function(serverMod.inst(), "_start");

    pthread_t c_th, s_th;
    wamr::start_wasm_thread(clientMod.inst(), client_start, &c_th);
    wamr::start_wasm_thread(serverMod.inst(), server_start, &s_th);
